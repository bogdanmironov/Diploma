...
def train_model(hyperparameters, log_dir):
    ...
    model = tf.keras.models.Sequential(
        [
            Conv1D(320, 8, strides=1, activation='relu', input_shape=(1000, 4), padding='valid', kernel_constraint=max_norm(2)),
            MaxPooling1D(4, strides=4),
            Dropout(0.2),
            Conv1D(480, 8, strides=1, activation='relu', padding='valid'),
            MaxPooling1D(4, strides=4),
            Dropout(0.2),
            Conv1D(960, 8, strides=1, activation='relu', padding='valid'),
            Dropout(0.5),
            Flatten(),
    ...
    for learning_rate in LEARNING_RATE:
        for l0_dropout_rate in L0_DROPOUT_RATES:
            for l1_dropout_rate in L1_DROPOUT_RATES:
                ...
                log_dir = './logs/Convoltuion/' + 'asdfasdf'
