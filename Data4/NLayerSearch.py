...
def train_model(hyperparameters, log_dir):
    ...
    my_callbacks = [
        ...
        EarlyStopping('val_auc', patience=70, mode='max')
    ]
    ...
    model = tf.keras.models.Sequential(
        [
            ...
            Dense(hyperparameters['l2_hu'], activation=tf.keras.activations.relu),
            Dropout(hyperparameters['l2_dr']),
            Dense(hyperparameters['l3_hu'], activation=tf.keras.activations.relu),
            Dropout(hyperparameters['l3_dr']),
            Dense(hyperparameters['l4_hu'], activation=tf.keras.activations.relu),
            Dropout(hyperparameters['l4_dr']),
...
dr = [0.00, 0.10, 0.30, 0.40, 0.45, 0.50]

for optimizer in OPTIMIZERS:
    for i in range(100, 1301, 300):
        for j in dr:
            ...
            log_dir = 'logs/log4layer_dr01234/' + str(optimizer) + '/lr='+ str(learning_rate) + '/' + '[' + str(hparams['l0_dr']) + ']'  + str(hparams['l1_hu']) + '[' + str(hparams['l1_dr']) + ']' + str(hparams['l2_hu']) + '[' + str(hparams['l2_dr']) + ']' + str(hparams['l3_hu']) + '[' + str(hparams['l3_dr']) + ']' + str(hparams['l4_hu']) + '[' + str(hparams['l4_dr']) + ']'
